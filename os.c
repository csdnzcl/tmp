...
static ssize_t my_read(struct file *file, char __user *buf, size_t count, loff_t *ppos) {
    ...
    if (kfifo_is_empty(&myfifo)) {
        if (file->f_flags & O_NONBLOCK)
            return -EAGAIN;
        wait_event_interruptible(read_queue, !kfifo_is_empty(&myfifo));
    }
    if (kfifo_to_user(&myfifo, buf, count, &copied))
        return -EFAULT;
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos) {
    ...
    if (kfifo_avail(&myfifo) == 0) {
        if (file->f_flags & O_NONBLOCK)
            return -EAGAIN;
        wait_event_interruptible(write_queue, kfifo_avail(&myfifo) > 0);
    }
    if (kfifo_from_user(&myfifo, buf, count, &copied))
        return -EFAULT;
